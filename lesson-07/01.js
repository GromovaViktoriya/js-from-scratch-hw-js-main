...
const imgSlot = document.getElementsByTagName('img')[0]
let currentImageIndex = 0

function imgSlotIndexed() {
    imgSlot.src = WEB_TECH_IMAGES[currentImageIndex]
}


prevBtn.addEventListener('click', () => {
    currentImageIndex = currentImageIndex - 1 < 0 ? WEB_TECH_IMAGES.length - 1 : currentImageIndex - 1
    imgSlotIndexed()
})

nextBtn.addEventListener('click', () => {
    currentImageIndex = currentImageIndex + 1 >= WEB_TECH_IMAGES.length ? 0 : currentImageIndex + 1
    imgSlotIndexed()
})